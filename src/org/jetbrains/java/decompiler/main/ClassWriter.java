...
public class ClassWriter {
        ...
        if (root != null && !methodWrapper.decompiledWithErrors) { // check for existence
          try {
            ...
            hideMethod = code.length() == 0 &&
              ...
              DecompilerContext.getOption(IFernflowerPreferences.EXCLUDE_RECORD_METHODS) &&
                (isAutogeneratedGetter(cl, mt, code) || isAutogeneratedConstructor(cl, mt, code));
  ...
  private boolean isAutogeneratedGetter(StructClass structClass, StructMethod structMethod, TextBuffer code) {
    if (structClass.getRecordComponents() == null) {
      return false;
    }
    String methodName = structMethod.getName();
    String methodDescriptor = structMethod.getDescriptor();
    Optional<StructRecordComponent> correspondingRecord = structClass.getRecordComponents().stream()
      .filter(component -> component.getName().equals(methodName))
      .findFirst();
    if (correspondingRecord.isPresent()) {
      String descriptor = correspondingRecord.get().getDescriptor();
      if (descriptor != null && methodDescriptor.equals("()" + descriptor)) {
        if (code.countLines() == 1) {
          String str = code.toString().trim();
          return str.startsWith("return this." + methodName);
        }
      }
    }
    return false;
  ...
  private boolean isAutogeneratedConstructor(StructClass structClass, StructMethod structMethod, TextBuffer code) {
    if (structClass.getRecordComponents() == null) {
      return false;
    }
    ...
    if (CodeConstants.INIT_NAME.equals(methodName)) {
      ...
      descriptor.append("(");
      for (StructField f : structClass.getFields()) {
        descriptor.append(f.getDescriptor());
      }
      descriptor.append(")V");
      if (descriptor.toString().equals(methodDescriptor)) {
        if (code.countLines() == structClass.getFields().size()) {
          for (StructField f : structClass.getFields()) {
